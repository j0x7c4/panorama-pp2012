...
feature getFeature(IplImage *src,double sigma,int masksize,int R){
    ...
    IplImage *img2=0;
    img2=cvCreateImage(cvSize(src->width,src->height),IPL_DEPTH_8U,3);
    img2=cvCloneImage(src);

    

    IplImage *img_s=0;
    img_s = cvCreateImage(cvSize(img->width,img->height),IPL_DEPTH_8U,1);
    CvMat ImageData;
    ...
    X_gradient = cvCreateMat(img->height,img->width,CV_32FC1); 
    Y_gradient = cvCreateMat(img->height,img->width,CV_32FC1);
    XY_gradient = cvCreateMat(img->height,img->width,CV_32FC1);
    
    X_gradient_for_dis = cvCreateMat(img->height,img->width,CV_32FC1); 
    Y_gradient_for_dis = cvCreateMat(img->height,img->width,CV_32FC1);
    ...
    CvMat *F_MAP=0;
    F_MAP = cvCreateMat(img->height,img->width,CV_32FC1);
    ...
    L_max = cvCreateMat(img->height,img->width,CV_32FC1);
    
    IplImage *test=0;
    test = cvCreateImage(cvSize(img->width,img->height),IPL_DEPTH_8U,1);
    
    for(int i=0;i<img->height;i++){
        for(int j=0;j<img->width;j++){
            DT->data.fl[0]=X_gradient->data.fl[i*X_gradient->cols+j]; 
            DT->data.fl[1]=XY_gradient->data.fl[i*XY_gradient->cols+j];
            DT->data.fl[2]=Y_gradient->data.fl[i*Y_gradient->cols+j];
            DT->data.fl[3]=XY_gradient->data.fl[i*XY_gradient->cols+j];
            ...
            F_MAP->data.fl[i*F_MAP->cols+j] = thre;
            L_max->data.fl[i*L_max->cols+j] = 0;
        }
    }
...
IplImage* connect_and_blend(IplImage *left_src,IplImage *right_src,int cols,int y){
    ...
    IplImage *dst=0;
    IplImage *temp_left=0,*temp_right;
    int left_col=0;
    left_col =  left_src->width-cols;
    
	
	int max_h=0;
	max_h=max(left_src->height,right_src->height);

	temp_left = cvCreateImage(cvSize(left_src->width,max_h+(int)abs(y)),IPL_DEPTH_8U,3);
	temp_right = cvCreateImage(cvSize(right_src->width,max_h+(int)abs(y)),IPL_DEPTH_8U,3);
	cvSetZero(temp_left);
	cvSetZero(temp_right);

	if(y <= 0){
		cvSetImageROI(temp_left,cvRect(0,abs(y),temp_left->width,left_src->height));
		cvCopy(left_src,temp_left,NULL);
		cvResetImageROI(temp_left);

		cvSetImageROI(temp_right,cvRect(0,0,temp_right->width,right_src->height));
		cvCopy(right_src,temp_right,NULL);
		cvResetImageROI(temp_right);

	}
	else if( y > 0){
		cvSetImageROI(temp_left,cvRect(0,0,temp_left->width,left_src->height));
		cvCopy(left_src,temp_left,NULL);
		cvResetImageROI(temp_left);
		
		cvSetImageROI(temp_right,cvRect(0,abs(y),temp_right->width,right_src->height));
		cvCopy(right_src,temp_right,NULL);
		cvResetImageROI(temp_right);
	}
	

	dst =cvCreateImage(cvSize(left_src->width+right_src->width-cols,max_h+(int)abs(y)),IPL_DEPTH_8U,3);
    

    for(int i=0;i<dst->height;i++){
        for(int j=0;j<dst->width;j++){
            if(j<left_col){
                dst->imageData[i*dst->widthStep+j*3]=temp_left->imageData[i*temp_left->widthStep+j*3];
                dst->imageData[i*dst->widthStep+j*3+1]=temp_left->imageData[i*temp_left->widthStep+j*3+1];
                dst->imageData[i*dst->widthStep+j*3+2]=temp_left->imageData[i*temp_left->widthStep+j*3+2];
            }
            else if(j>=left_src->width){
                dst->imageData[i*dst->widthStep+j*3]  =temp_right->imageData[i*temp_right->widthStep+(j-left_col)*3];
                dst->imageData[i*dst->widthStep+j*3+1]=temp_right->imageData[i*temp_right->widthStep+(j-left_col)*3+1];
                dst->imageData[i*dst->widthStep+j*3+2]=temp_right->imageData[i*temp_right->widthStep+(j-left_col)*3+2];
				
            }
            else{
                double a=0;
                a= (double)(j-left_col+1)/(double)cols;
                
                dst->imageData[i*dst->widthStep+j*3]=(int)((double)((unsigned char)temp_left->imageData[i*temp_left->widthStep+j*3]) * (1-a) +
                                                     (double)((unsigned char)temp_right->imageData[i*temp_right->widthStep+(j-left_col+1)*3]) * a);
                                                     
                dst->imageData[i*dst->widthStep+j*3+1]=(int)((double)((unsigned char)temp_left->imageData[i*temp_left->widthStep+j*3+1]) * (1-a) +
                                                     (double)((unsigned char)temp_right->imageData[i*temp_right->widthStep+(j-left_col+1)*3+1]) * a);
                                                     
                dst->imageData[i*dst->widthStep+j*3+2]=(int)((double)((unsigned char)temp_left->imageData[i*temp_left->widthStep+j*3+2]) * (1-a) +
                                                     (double)((unsigned char)temp_right->imageData[i*temp_right->widthStep+(j-left_col+1)*3+2]) * a);

            }
        }
    }
    cvReleaseImage(&temp_left);
    cvReleaseImage(&temp_right);
	return dst;
}
